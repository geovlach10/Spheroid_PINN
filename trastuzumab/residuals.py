...
## residual operators.
def pde(net: MLP | ConstrainedNet, dataset: Dataset, L=1.0, scaled: bool = False) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def center_neumann(net: MLP | ConstrainedNet, dataset: Dataset, target=0.0) -> torch.Tensor:
    r = dataset.data.clone()[:,0:1].requires_grad_(True)
    t = dataset.data.clone()[:,1:2].requires_grad_(True)
...
def surface_robin(net: MLP | ConstrainedNet, dataset: Dataset) -> torch.Tensor:
    r = dataset.data.clone()[:,0:1].requires_grad_(True)
    t = dataset.data.clone()[:,1:2].requires_grad_(True)
...
def initial(net: MLP | ConstrainedNet, dataset: Dataset, ic_func: Callable[[torch.Tensor], torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    r = dataset.data.clone()[:,0:1].requires_grad_(True)
    t = dataset.data.clone()[:,1:2].requires_grad_(True)
